...
class _CourseDetailPage extends State<CourseDetailPage> with TickerProviderStateMixin {
 ...
 CourseContentModel item2 = CourseContentModel();
   ...
   _courseModel = CourseModel();
    ...
    UserModel teacherModel = UserModel(userName: "张三",userDesc: "这个人很赖",avatarImageUrl: "https://img-blog.csdnimg.cn/20190902174921871.png?x-oss-process=image/watermark,type_ZmFuZ3poZW5naGVpdGk,shadow_10,text_aHR0cHM6Ly9ibG9nLmNzZG4ubmV0L01yc19jaGVucw==,size_16,color_FFFFFF,t_70");
    _courseModel.teacherMode = teacherModel;
    _tabPages = [CourseIntroducePage(_courseModel),CourseConetentPage(itemModels,true)];
    ...
    return Scaffold(
        ...
        slivers: <Widget>[
          SliverAppBar(
            ...
            flexibleSpace: new FlexibleSpaceBar(
              background: Image.network(_courseModel.imageUrl,fit: BoxFit.fill,),
            ),
          ),
      ...
      ).putIntoContainer(),bottom: 44,top: 0,left: 0,right: 0)
      .addSubWight(
        ...
        ).putIntoContainer(margin: EdgeInsets.only(left: 5,right: 5),width: 100))
        .addSubWight(FlatButton.icon(
          textColor: Colors.white,
          color: Colors.orange,
           shape:RoundedRectangleBorder(borderRadius: BorderRadius.circular(20.0)),
        ...
        ).putIntoContainer(margin: EdgeInsets.only(right: 5)),flex: 1)
        .putIntoContainer(height:44,width: double.infinity),bottom: 0,left: 0,right: 0) //()
    ).putIntoContainer(height: double.infinity).putInfoSaveArea(top: false);
  }
}
...
class CourseIntroducePage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return  SliverToBoxAdapter(
      child: Column().addSubWight(
        Text(_courseModel.courseTitle  == null ? "":_courseModel.courseTitle,textAlign: TextAlign.left,style: TextStyle(fontSize: 20),)
        .putIntoContainer(width: double.infinity,height: 30,margin: EdgeInsets.only(top: 10))
      )
      .addSubWight(Text(_courseModel.favouriteCount  == null ? "":_courseModel.favouriteCount.toString()+" 人收藏",textAlign: TextAlign.left,style: TextStyle(fontSize: 13),)
        .putIntoContainer(width: double.infinity,height: 30,margin: EdgeInsets.only(top: 10)))
      .addSubWight(getSection("简介", getIntroduce(_courseModel)).putIntoContainer(margin: EdgeInsets.only(bottom: 20)))
      .addSubWight(getSection("适合人群", Text(_courseModel.courseFor == null ? "":_courseModel.courseFor)))
      .addSubWight(getSection("老师",UserInfoItem(_courseModel.teacherMode)))
...
Column getSection(String title,Widget widget){
  return Column().addSubWight(
    Text(title,style: TextStyle(fontSize: 20),).putIntoContainer(width:double.infinity,height:30)
  )
  .addSubWight(widget.putIntoContainer(width: double.infinity,margin: EdgeInsets.only(bottom: 20)));
}

Column getIntroduce(CourseModel courseModel){
  
  Column descView = Column(
    key: PageStorageKey("courseIndtroduce"),
  ).addSubWight(Text(courseModel.courseDesc == null ? "":courseModel.courseDesc,style: TextStyle(fontSize: 18),).putIntoContainer(width: double.infinity));
  if(courseModel.courseDescImageList != null){
    for (var i = 0; i < courseModel.courseDescImageList.length; i++) {
   descView = descView.addSubWight(Image.network(courseModel.courseDescImageList[i]).putIntoContainer(width: double.infinity));
  }
  }
...
class CourseConetentPage extends StatefulWidget{
  ...
  CourseConetentPage(List<CourseContentModel> itemModels,bool isShowInSliverList){
...
@override
  State<StatefulWidget> createState() {
    ...
    return _CourseConetentPage(_itemModels,this.isShowInSliverList);
  }
...
bool isShowInSliverList;
  ...
  // _CourseConetentPage(List<CourseContentItemModel> itemModels){
  //   _itemModels = itemModels;
  // }
  
  _CourseConetentPage(List<CourseContentModel> itemModels,bool isShowInSliverList){
    _itemModels = itemModels;
    this.isShowInSliverList = isShowInSliverList;
  }
  ...
  @override
  Widget build(BuildContext context) {
    if(this.isShowInSliverList !=null && this.isShowInSliverList == true){
return SliverList(
      key: PageStorageKey("courseContent"),
    ...
    return ExpansionPanelList(
      ...
      children: _data.map<ExpansionPanel>((Item item) {
        return ExpansionPanel(
          ...
          body: Column(
            children: getSubItem(item.courseMoel).map((CourseContentItemModel subItem){
             return Column().addSubWight(
               Row().addSubWight(Icon(Icons.video_library))
              .addSubWight(Text(subItem.title == null ? "":subItem.title,style: TextStyle(fontSize: 15),).putIntoContainer(margin: EdgeInsets.only(left: 5)))
              .putIntoContainer(width: double.infinity,height: 60,padding: EdgeInsets.only(left:15))
             ).addSubWight(
               Container(color: Colors.black12,height: 1,margin: EdgeInsets.only(left: 15,right: 20),)
             )
               ...
               onTapDown: (event){
                    //  String url = "https://www.w3cschool.cn/statics/demosource/movie.mp4";
                        Navigator.push(context, MaterialPageRoute(builder: (context){
                          return VideoScreen(url: subItem.videoUrl,);
                        }));
               },
             ));
